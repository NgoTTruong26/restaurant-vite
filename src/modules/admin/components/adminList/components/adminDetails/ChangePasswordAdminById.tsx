...
import useChangePasswordAdminById from '../hooks/useChangePasswordAdminById';
import {
  InputChangePasswordAdminById,
  useFormChangePasswordAdminById,
} from '../hooks/useFormChangePasswordAdminById';
...
const ChangePasswordAdminById: React.FC<Props> = ({
  handleCloseShowChangePassword,
  data,
}) => {
  const { methods, formState } = useFormChangePasswordAdminById();

  const { mutate } = useChangePasswordAdminById();

  const ref = useRef<HTMLDivElement>(null);

  const onSubmit = (dataInput: InputChangePasswordAdminById) => {
    mutate(
      {
        id: data.id,
...
export default ChangePasswordAdminById;
